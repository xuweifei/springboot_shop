package com.opengroup.longmao.gwcommon.tools.sdk.qiniu;
...
//	private final static Auth auth = Auth.create(accessKey, secretKey);	
	 ...
	 * @throws
	 */
	public String downloadUrlLong(String key) throws Exception {
		String url = domainUrl();
		if (!url.endsWith("/")) {
			url += "/";
		}
...
//		String downloadRUL = getUpToken().privateDownloadUrl(path, 3600);
	...
	public StringMap uploadFile(byte[] data,String key) throws Exception{
		...
		GwsLogger.info("将文件返回信息，res={}:",JSON.toJSONString(res));
	...
	public void deleteFile(String key) throws Exception {
		key = key.replace(qiNiuConfig.getQiniuDomainName(), "").replace("/", "");
		GwsLogger.info("删除七牛文件:key={}", key);

...
public interface OrderInfoRepositorySlave extends BaseRepository<OrderInfo, Long> {
	 ...
	 */
	@Query("SELECT o FROM OrderInfo o WHERE o.buyerUid = ?1 AND o.sellerUid = ?2 AND o.chlId = ?3 AND o.orderStatus = ?4 AND o.deliveryStatus = ?5")
	List<OrderInfo> getOrderInfoListByBuyerUid(Long buyerUid, Long sellerUid, Long chlId, Short orderStatus, Short deliveryStatus);

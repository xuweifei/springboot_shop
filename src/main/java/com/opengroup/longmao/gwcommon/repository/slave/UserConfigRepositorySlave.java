package com.opengroup.longmao.gwcommon.repository.slave;

import java.util.List;

import org.springframework.data.jpa.repository.Query;

import com.opengroup.longmao.gwcommon.configuration.query.core.BaseRepository;
import com.opengroup.longmao.gwcommon.entity.po.UserConfig;

/**
 * 【用户配置表】 RepositorySlave接口
 * @version 1.0
 * @author Hermit 2017年03月15日 上午09:59:49
 */ 
public interface UserConfigRepositorySlave extends BaseRepository<UserConfig, Long> {

	@Query("select u from UserConfig u where u.configPrefix = ?1 ")
	List<UserConfig> getInviteConfig(String configPrefix);
	
	
	
	
	
}


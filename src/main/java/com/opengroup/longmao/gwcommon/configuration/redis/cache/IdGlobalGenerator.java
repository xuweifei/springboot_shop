...
public class IdGlobalGenerator {
	...
	public Long getSeqId(String prefix) {
		Long seqId = null;
		try {
			seqId = getCacheSeqIncr(prefix);
		} catch (Exception e) {
	...
	public Long getSeqId(String prefix, Long id) {
		Long seqId = null;
		try {
			seqId = getCacheSeqIncr(prefix, id);
		} catch (Exception e) {
		}
		if (null == seqId) {
			seqId = getCacheSeq();
	...
	private Long getCacheSeq() {
		Long maxNum = System.currentTimeMillis();
		Long randNum = getRandNum();
		return maxNum / 100000 + randNum;
	}
	...
	private  Long getCacheSeqIncr(String prefix, Long id){
		 counter = new RedisAtomicLong(CacheConstant.CACHE_ID_GEN + prefix,
				 redisClient.getConnectionFactory(), id);
		 return counter.incrementAndGet();
	}
